...
export const ChatSidebar = memo(({
  ...
  const regularChats = useMemo(() => 
    chats.filter((c) => c.id !== AUTO_ASSIST_ID),
    [chats]
  )

  // アシスタント選択ハンドラーのメモ化
  const handleChatSelect = useCallback((id: number) => {
    onSelectChat(id)
  }, [onSelectChat])
  ...
  // 編集ハンドラーのメモ化
  const handleEditClick = useCallback((chat: ChatInfo) => {
  ...
  const handleDrop = useCallback((e: React.DragEvent<HTMLLIElement>, index: number) => {
    onDrop(e, index)
  }, [onDrop])
  ...
  const autoAssistStatusBadge = useMemo(() => {
    if (autoAssistState === 'awaitConfirm') {
      return <Badge colorScheme="yellow" variant="solid" fontSize="xs">確認待ち</Badge>
    }
    if (autoAssistState === 'executing') {
      return <Badge colorScheme="blue" variant="solid" fontSize="xs">実行中</Badge>
    }
    return null
  }, [autoAssistState])
    ...
    >
      ...
      <Box p={4} borderBottom="1px solid #ddd" bg="white">
        <HStack justify="space-between" align="center">
          <Text fontSize="lg" fontWeight="bold">アシスタント一覧</Text>
          <Menu>
            ...
            <MenuList>
              ...
              {isExternalApiEnabled && (
                <MenuItem onClick={onShowAPISettings}>API設定</MenuItem>
              )}
